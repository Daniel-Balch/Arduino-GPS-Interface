...
int initUSART(void) {                                /* requires BAUD */
...
int printString(const char myString[]) {
  uint8_t i = 0;
  while (myString[i]) {
...
int readString(char myString[], uint8_t maxLength) {
  char response;
  uint8_t i;
  i = 0;
...
int printBinaryByte(uint8_t byte) {
